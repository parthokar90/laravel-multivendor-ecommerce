...
@section('title') Brand Product @endsection
...
<section class="relative bg-cover bg-center py-20 text-white" style="background-image: url('{{ asset('front/assets/images/banner.jpg') }}');">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-3xl md:text-4xl font-bold tracking-tight mb-3">Brand Product</h2>
            ...
            <div class="w-full">
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl border border-stone-200 px-6 py-4 flex justify-between items-center shadow-sm">
                        <h4 class="text-sm font-bold uppercase tracking-wider text-stone-700">Home / Brand</h4>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        ...
                        <div class="group bg-white rounded-xl border border-stone-200 overflow-hidden shadow-sm hover:shadow-md transition duration-300 flex flex-col justify-between">
                            ...
                            <div class="relative bg-stone-50 overflow-hidden aspect-square flex items-center justify-center p-4">
                                <a href="{{ route('product.single', ['id' => $item->id, 'slug' => $item->product_slug]) }}" class="w-full h-full block">
                                    <img src="{{ $item->image }}" class="w-full h-full object-cover rounded-lg group-hover:scale-105 transition duration-500" alt="{{ $item->product_name }}" />
                                </a>
                                ...
                                <div class="absolute bottom-3 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-xl shadow-lg border border-stone-100 flex items-center space-x-4 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition duration-300">
                                    <form method="post" action="{{ route('wishlist.store') }}" class="inline-block">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $item->id }}">
                                        <button type="submit" class="text-stone-600 hover:text-rose-500 transition relative group/tooltip" title="Add to Wishlist">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                                    ...
                                    <a href="{{ route('product.single', ['id' => $item->id, 'slug' => $item->product_slug]) }}" class="text-stone-600 hover:text-stone-900 transition" title="Quick View">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                            ...
                            <div class="p-5 flex-1 flex flex-col justify-between space-y-3">
                                <div class="space-y-1.5">
                                    ...
                                    <a href="{{ route('product.single', ['id' => $item->id, 'slug' => $item->product_slug]) }}" class="block">
                                    ...
                                    <p class="text-sm font-medium text-stone-800">
                                        <span class="text-xs text-stone-400 line-through mr-1.5">$110</span> $78
                                    </p>
                                </div>

                                <div class="pt-2">
                                    <a href="{{ route('product.single', ['id' => $item->id, 'slug' => $item->product_slug]) }}"
